...
Fictional mining and resources employer in the WorkReady internship simulation. This repository contains its public website, careers/application pages, staff area and character content.
...
| Path | Purpose |
|---|---|
| `brief.yaml` | Company profile and website context |
...
| `content/docs/` | Fictional policies and support materials, including FIFO and mining scenarios |
| `content/jobs/`, `jobs.json` | Job authoring material and exported build input |
| `dist/` | Generated website published to GitHub Pages |

The workplace policies are teaching content. WorkReady's actual privacy and operating rules live in the project guides linked above.
...
Ensure `jobs.json` agrees with the canonical `workready-api/jobs/ironvale-resources.json` export. If the company-root copy is missing from a fresh checkout, copy that API export here before building. Editing `brief.yaml` or job Markdown does not regenerate the export.
