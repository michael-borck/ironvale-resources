...
"""Build the IronVale Resources bespoke site.
...
Usage:
    ...
    ./site/build.py serve        # build then serve on :8001 for local preview

Requires `uv` to be installed (https://docs.astral.sh/uv/) — pyyaml and
jinja2 are pulled in via the shebang.
"""
...
def build():
    print(f"Building IronVale Resources → {DIST_DIR}")
    ...
    pages = [
        ...
        ("operations.html.j2", "operations.html"),
    ...
    if not cname_file.exists():
        cname_file.write_text("ironvaleresources.eduserver.au\n", encoding="utf-8")
        print("  ✓ CNAME")
...
def serve():
    """Build then serve the dist directory on :8001 for local preview."""
    import http.server
    import socketserver
    import os
    ...
    with socketserver.TCPServer(("", PORT), http.server.SimpleHTTPRequestHandler) as httpd:
        print(f"\nServing IronVale at http://localhost:{PORT}")
